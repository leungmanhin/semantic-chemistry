...
| `qa_gate.py` | The query-side admission bar over `qa_parsed.json`: every query symbol exists in the pooled knowledge and the question's focus grounds (`ok` / `focus-in-rules-only` / `focus-not-grounded` / `unmatched-symbol`); an N premise fires at least one world-rules law; a C intervention matches the pool and names the laws its removal cuts. `--mock` exercises it on records mocked from the parser's perspective | n/a |
