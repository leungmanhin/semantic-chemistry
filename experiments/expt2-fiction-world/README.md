...
| `lore.md` | Background in the CONTROLLED LANGUAGE (LORE register, v2 — the batch-1 pass after gate run 1): 105 paragraph entries across the ten sections; generated from `lore.json` by `regen_lore_md.py` (`>` lines = design annotations from `lore_notes.json`) | 610 |
| `lore.json` | **Machine-facing parse input** for the lore — entries `{id, rule, texts}` (`id` = section-paragraph, e.g. `L5-02`; `rule` = section label); the SOURCE OF TRUTH for lore text. | 610 |
...
`world_rules` is ADMITTED: all 107 sentences pass the fireability census (five gate runs, incremental admission from run 5); `world_rules_parses.json` is the accepted parse record and the reviewer's residual remarks live in `expected_consolidation_map.md`'s advisory backlog. `lore` is ADMITTED on the mechanical bar after two gate runs: run 1 parsed all 607 sentences census-clean, batch 1 rewrote the silently mis-parsing shapes (the `lint_corpus.py` classes, the empty-parse sentence, four QA-critical laws) and run 2 re-parsed those 87 sentences census-clean and non-empty — `lore_parsed.json` is the assembled accepted record (610 sentences). The reviewer's residual remarks on lore (compounds and container phrases, coordinated lists, frequency adverbs on the Council, "the village" left anonymous, the "When"-variant occasion keying) are batch-2 material under the convergence protocol. `events.md` remains free prose pending its controlled re-skin (EPISODIC register, parsed per entry as a passage). Later expansion with LLM-generated paraphrase variants goes through the same parse-gate.
