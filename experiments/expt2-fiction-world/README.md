...
`world_rules` is ADMITTED: all 107 sentences pass the fireability census (five gate runs, incremental admission from run 5); `world_rules_parses.json` is the accepted parse record and the reviewer's residual remarks live in `expected_consolidation_map.md`'s advisory backlog. `lore` is ADMITTED after four gate runs (`lore_parsed.json`, 613 sentences census-clean); one sentence added for the events firing test, "Hesper is a Keeper.", waits in `lore_pending.json`. `events` is re-skinned into the EPISODIC register (83 episodes, 687 sentences, `events.json`, including nine routine-night episodes that each close a full loop turn inside one passage); it is ADMITTED on the mechanical bar after two gate runs per entry as a passage (`events_parsed.json`, 83 passages: 81 census `ok` and 2 whose only flagged rules sit inside sealed reports, which the assembler exempts as `ok (sealed)`). A unifier test of every world-rules premise against the pooled atoms fired 31 of 89 laws on run 2; the silent core laws traced to instance-vocabulary gaps, now closed by a batch-3 authoring pass (`events_pending.json` holds its 24 changed episodes, `lore_pending.json` the one lore fact it needs, "Hesper is a Keeper."), and to the ingestion conventions listed in `REALIGNMENT.md` item 1. The QA file dates Coraline Ash's investiture to the spring of Year 2 (F30) while the corpus calendar puts it on day 260 of Year 1; the QA line is the one to fix. Later expansion with LLM-generated paraphrase variants goes through the same parse-gate.
