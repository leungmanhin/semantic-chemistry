...
| `demo-scaffold.metta` | the end-to-end **derived-control** run, and the one that carries the arc furthest: two arrivals worked by the hand-made chain → the scan finds it recurrent, useful and **not closed** → ctrl-scaffold bills the parents and mints a recombinant offspring wrapping it in a control level, whose genome is printed as ordinary `rule-lhs`/`rule-rhs` IR → the offspring works its own stream and the same detector certifies it on **all five conditions at once**, purse growing. Then the catch: run the identical genome from the identical endowment under the leaner `CL_deep` and a turn costs more than it earns, so on the third arrival it cannot afford `prime` — the closing edge is the first thing it loses. Certification is regime-relative, and closure and viability can disagree |
| `README.md` | this file |
