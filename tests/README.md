...
Each `*_test.metta` file loads the Exp-1 data (`../experiments/expt1-causal-qa/`) + the engine and its stage's logic module (`../petta/`), then wires a scenario + its own `!(test …)` assertions — so it **both demonstrates** a stage of the stack **and verifies** it. The *generic* mechanism lives in `../petta/` (`util`/`fuel`/`compiler`/`gate`/`reaction` · `evaluator` · `persist`/`chamber`/`reducer`/`workers` · `acs` · `ecan` · `selection`); the test file is thin (import + scenario + asserts). A failing `!(test)` prints `is X, should Y. ❌` and halts that file (`../../PeTTa/src/metta.pl`). The reference for correctness is **Ben's `docs/` PDFs**.
...
Runs every `tests/*.metta` from the repo root (imports are repo-root-relative), tallies ✅/❌ per file, and exits non-zero on any failure. **Suite: 120 tests green.** One file at a time:
...
- ✅ **worker kinds + verdict facts** (`worker_kinds_test.metta`, 11; logic `petta/workers`+`acs`/`ecan`/`selection`) — the analysis passes dispatched **through the §6.4 contract** as `worker-kind`s: `dispatch` routes `acs-scan`→`run-acs`, `ecan-epoch`→`run-epoch`, `reproduction`→`run-select` (beside `context-chamber`/`reducer`). Each asserts its **§2.9/§2.10 verdict facts** to `&self` and persists them to `runs/<id>.metta`: acs-scan → `acs-member`/`acs-closure`/`acs-surplus`/`acs-promoted` (3 members, +6, promoted); reproduction → `genome-fitness`/`birth`/`lineage-improvement` (founder −3, pruned child +6, +9).
