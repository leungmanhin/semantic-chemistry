...
| File | What |
|------|------|
| `kernel.metta` | the generic **ENGINE** (facts-free, `$ws`-parameterized, re-run-safe): a **per-organism** typed-fuel ledger (`fuel O tok n`, schema §2.3) · componentwise affordability · the **computed gate** `gate?` = `rule-eligible?` ∧ `afford` ∧ `attn-ok?` (MSC eq 24 / §3 step 3) driving fire + event log · the per-rule `walk`; **organism resolution** (`rule-organism`/`the-organism` — a rule → its owning organism via ruleset membership) and the **typed-attention gate** (`attn-ok?` over `(attention R tok v)`); the **canonical-IR interpreter** (`clause-join`/`rule-cands`); the **converter** (`to-vars`/`express`/`convert-rule` — `(var Name)` → co-referent match-vars); the **context gate** (`rule-eligible?` — a rule reaches the walk only if its `rule-context` is a declared `chamber-context`); and the generic data-driven **`run-rules`** / **`run-ablate`** (walk by `rule-priority`, optionally skipping a set). Set-semantics `add-all` (the doc's ⊕). The per-organism ledger, attention, and `gate?` are degenerate on single-organism Exp-1 — the attention dynamics and inter-organism economy are not modelled. NO domain logic, NO data. |
