...
**Suite: 77 tests green** across the modules below.

## Status (P0–P4, canonical per-match)

- ✅ **P0 unified mortal kernel** (`kernel.metta`, 10 ✅) — the generic engine drives the genome expressed from its canonical IR. Seeded gm 27 / ci 7 it exhibits **P0 mortality: 27 firings, 5 answers, Q7b STARVES** (aligned but unprojected). Per-match: `R_align` fires once **per edge** (7 cites; Q23 cites both causes — use *and* overmix — separately); **abstain is no soma rule** (Q20 produces nothing — the alignment rule simply doesn't match its focus); set-add dedups the shared alignment head.
- ✅ **P1 evaluator** (`evaluator.metta`, 6 ✅) — eq-17 **Match** scoring (graph-derived: `Match = covered/idealsize`, `floor-math` mint round) + the **clamp-switch**: gm-income flips **25 (physical) ↔ 16 (intentional)**. `Q3a` mint 3 (Match 0.5, via the transitive-`CauseOf` ideal), `Q23` 6 (2/2). **Abstain is the evaluator's job now**: `abstain-income` mints 4 for a question whose graph-ideal is empty and the soma left unanswered (gated by `typed?`, so a broken genome earns no spurious abstain reward).
- ✅ **P4.2 worker ecology** (`workers.metta`, 10 ✅) — **separate-space-per-worker** isolation (chosen on the `spaces_behavior.metta` battle-test): each `context-chamber` worker gets its own `(new-space)`; the read-only IR stays shared in `&self`; the engine is `$ws`-parameterized. Two workers over the same IR, different fuel snapshots → isolated mortal trajectories (`W_full` gm 28/ci 7 → 28 firings/6 answers; `W_lean` gm 8 → starves at 8/0), and a `reducer` merges their summaries (36 firings, 6 answers).
- ✅ **P4.3(i) ecan-epoch** (`ecan_epoch.metta`, 6 ✅) — the **P1 metabolic-closure loop**: run → score+mint+credit → re-run while there's progress + fuel (re-run-safe via the fire-once `walk` + seq threading). At a constrained seed (gm 23): feedback **OFF** = 1 answer, 23 firings, starves; **ON** = **6 answers, 28 firings, ends at gm 16 (surplus)** — minting funds the further answers and the loop pays its own way.
- ✅ **P4.3(ii) acs-scan + replay** (`acs_scan.metta`, 16 ✅) — the **P2 mortal-ACS certification**. *Ablation* (cond 4): suppress one rule in its own space (generous fuel → structural) — base 6 answers; R_qtype/R_align/R_project load-bearing (→0/0/0), **R_complete INERT (→6, 0 impact)** = the dead-weight rule P3 prunes. *Closure* (cond 1+2): reachability on the rule-motif graph — a closed metabolic cycle exists **only with the evaluator's E→fuel edge** (True), not among the soma rules alone (False) ⇒ closure is *metabolic*. *Surplus* (cond 3) + *promotion*: the FULL genome runs a **DEFICIT** (income 25 < spend 28 — R_complete's 9 firings are dead weight); the autocatalytic **CORE** (R_complete pruned) spends only 19, so income − spend = **+6 under CL_antecedent vs −3 under CL_goal**, and the same core is **PROMOTED under one clamp, REJECTED under the other** (clamp-switch at the ACS level).
- ✅ **P4.3(iii) selection-epoch** (`selection.metta`, 9 ✅) — the **P3 lineage**. Express a genome = run only its member rules (generic `run-ablate` skipping the non-members); fitness = metabolic surplus. Founder {qtype,align,complete,project} = **−3** (the inert R_complete's 9 firings sink it into deficit); **pruning R_complete → +6** (the unique fitter, the only viable genome) so selection rediscovers it and the lineage steps GNM_qa(−3) → GNM_g1(+6); pruning any load-bearing rule collapses surplus (R_qtype 0, R_align −2, R_project −18). **Recombination** of two answer-incapable genomes (one missing R_project, one missing R_qtype) → the union genome is **viable again (6 answers)**.
...
| `acs_scan.metta` | **P4.3(ii)** — the P2 ACS certification: ablation (R_complete inert), metabolic-closure reachability, surplus (full-genome deficit; core +6/−3), promotion clamp-switch; 16 tests |
| `selection.metta` | **P4.3(iii)** — the P3 lineage: express a genome by membership, fitness = surplus, prune-R_complete is the fitter (−3→+6), recombination rescues; 9 tests |
