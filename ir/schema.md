...
| `(execution-model M)` | Which engine a run uses, `M ∈ {cascade, collect-then-fire}`; **undeclared = `cascade`**. `cascade` is the within-cycle priority walk — each firing's products are visible to the rules walked after it, so a whole chain can run in one cycle and `rule-priority` has to encode the genome's dependency order. `collect-then-fire` is **TECAN Alg 5.1**: the candidate queue is assembled in full (line 10) before the first rewrite (line 16), so every precondition is evaluated against the **epoch-start state**, nothing in the queue was enabled by anything else in it, and a chain advances **one hop per cycle**. Global rather than per-chamber, because a counterfactual replay must run the same engine as the run it is paired against. |
