...
| `(rule-do-influence R target answer-reward cycles n)` | Per-rule causal coding (SC §3.7): the **leave-one-out** paired replay — the drop in *cumulative* minted reward when rule `R` alone is suppressed, against an unablated run of the same length, same snapshot / budget / context. `cycles` is the **window** it was measured over, and is part of the claim: a rule whose product survives decay acts on the *next* cycle, so it reads `0` over one cycle and load-bearing over two, and both readings are true of their own window. The scan uses the observed log's cycle count **capped by `(acs-window-max n)`** (config, default 3), so the replay pairs with the run being certified on a short log and stays bounded on a long one — past the cycle where the ablated and unablated trajectories separate each further cycle only adds a constant, so a bigger window rescales the delta without adding information. Emitted per ACS member. |
