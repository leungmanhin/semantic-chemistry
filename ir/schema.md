...
> **Known IR evolution — `chamber-hot-rule` → typed attention + computed gate (deferred).** `(chamber-hot-rule C R)` is a **P0 boolean placeholder**: a degenerate collapse of "attention above the hot threshold," hand-asserted because Exp-1 has one chamber and no attention dynamics (the **attention** filter is inert here; the **context** filter is now implemented — `kernel_defs` `rule-eligible?`, which gates a rule to the walk only if its `rule-context` is a declared `chamber-context`, a no-op for single-context Exp-1 — so fuel + the context gate make the firing decision). Intended end-state, in two parts: (1) **stored *typed-attention* state** per worker — a mutable, per-worker fact shaped like `(fuel …)` (decays / pays rent), and **typed** per TECAN (scalar ECAN STI → typed, causally-priced attention), e.g. `(attention R token-type v)`; (2) the **computed gate** `Gate_C(R,O,X)` already specified at §2.6 (`event-gate`) and §3 step 3 (MSC eq 24) as the actual fire decision. So `chamber-hot-rule` does **not** become a stored scalar STI — it *dissolves into evaluating the gate* over rule facts (`rule-tv`, attention, fuel, ACS-membership), the way `afford` is computed over `fuel` facts. The remaining piece is the attention *dynamics* (what makes a rule hot vs cold); a hot-gate reading hand-asserted hotness would run but stay degenerate (all-hot) until those dynamics exist, so it lands with a multi-chamber experiment (Exp 2). The gate reads `rule-tv`/`rule-lhs` as data (now available).
